...
const CustomDialog = {
  show: ({ customContent, showCloseButton = true, backdropStyle = {} }) => {
    const dialogRoot = document.createElement('div');
    dialogRoot.id = 'custom-dialog-root';
  ...
  hide: () => {
    const dialogRoot = document.getElementById('custom-dialog-root');
    if (dialogRoot) {
...
const WhaleMovementAlerts = () => {
  ...
  const handleTelegramSubscribe = () => {
    window.open('https://t.me/pioneers1234', '_blank');
    setTelegramEnabled(true);
    toast.success('Successfully subscribed to Telegram notifications!', {
      icon: '🔔',
      duration: 4000
    });
  };
  ...
  const handleTelegramToggle = async (alert) => {
    try {
      const isSubscribed = notificationService.telegramSubscribers.has(alert.wallet);
      
      if (!isSubscribed) {
        notificationService.subscribeTelegram(alert.wallet);
          ...
          customContent: (
            ...
            >
              <Box sx={{ textAlign: 'center', mb: 3 }}>
                ...
                >
                  <TelegramIcon sx={{ fontSize: 48, color: '#60A5FA', mb: 2 }} />
                </motion.div>
                <Typography variant="h5" sx={{ color: '#fff', mb: 1 }}>
                  Subscription Activated! 🎉
                </Typography>
                <Typography variant="body1" sx={{ color: 'rgba(255, 255, 255, 0.8)', mb: 3 }}>
                  You will receive instant alerts for this wallet on our BigFish Whale Channel
                </Typography>
              </Box>
        ...
        notificationService.unsubscribeTelegram(alert.wallet);
        toast.success('Unsubscribed from Telegram alerts', {
          icon: '🔕',
          duration: 3000
        });
      }
    } catch (error) {
      console.error('Failed to toggle Telegram subscription:', error);
      toast.error('Failed to update Telegram subscription');
    }
  };
  ...
  useEffect(() => {
    ...
    notificationService.setTelegramService(telegramService);

    // Monitor whale movements
    const monitorWhaleMovements = async () => {
      try {
        const url = `https://api.unleashnfts.com/api/v2/nft/wallet/analytics?blockchain=${blockchain}&time_range=15m&sort_by=volume&sort_order=desc&offset=0&limit=30`;
        ...
        data.data.forEach(movement => {
          if (movement.volume >= threshold) {
            notificationService.notify(movement.wallet, {
              volume: movement.volume,
              nft_bought: movement.nft_bought,
              nft_sold: movement.nft_sold,
    ...
    const monitoringInterval = setInterval(monitorWhaleMovements, 60000);

    // Initial run
    monitorWhaleMovements();
  ...
  useEffect(() => {
    if (telegramEnabled) {
      ...
      const interval = setInterval(monitorWhaleMovements, 60000); // Check every minute
      monitorWhaleMovements(); // Initial check

      return () => clearInterval(interval);
    }
  }, [telegramEnabled, blockchain, threshold]);
  ...
  // Replace any router navigation with Next.js's router
  const handleViewOnOpenSea = (walletAddress) => {
      ...
      >
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', gap: 2 }}>
            <Tooltip title={telegramEnabled ? "Telegram notifications enabled" : "Enable Telegram notifications"}>
              <IconButton
                onClick={() => setShowTelegramDialog(true)}
                sx={{
                  color: telegramEnabled ? '#60A5FA' : '#fff',
                  background: telegramEnabled ? 'rgba(96, 165, 250, 0.2)' : 'transparent',
                  '&:hover': {
                      ...
                      <Grid container spacing={2} alignItems="center">
                        <Grid item xs={12} md={4}>
                          ...
                          <Typography 
                            ...
                            onClick={() => handleViewOnOpenSea(alert.wallet)}
                          >
                            {alert.wallet}
                          </Typography>
                        ...
                        <Grid item xs={12} md={2} sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
                          ...
                          <motion.div whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}>
                            <Tooltip title={notificationService.telegramSubscribers.has(alert.wallet) ? 
                              ...
                              <IconButton
                                ...
                                sx={{ 
                                  color: notificationService.telegramSubscribers.has(alert.wallet) ? '#60A5FA' : '#fff',
                                  background: notificationService.telegramSubscribers.has(alert.wallet) ? 
        ...
        >
          <DialogTitle>Join Our Telegram Channel</DialogTitle>
          <DialogContent>
            <Typography variant="body1" sx={{ mb: 2 }}>
              Get instant whale movement alerts on Telegram!
            </Typography>
            <Button
              variant="contained"
              startIcon={<TelegramIcon />}
          ...
          <DialogActions>
            <Button onClick={() => setShowTelegramDialog(false)} sx={{ color: '#fff' }}>
              Close
            </Button>
          </DialogActions>
        </Dialog>
